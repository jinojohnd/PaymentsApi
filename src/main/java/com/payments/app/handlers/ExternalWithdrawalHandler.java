...
public class ExternalWithdrawalHandler implements HttpHandler {
    ...
    private String processRequest(HttpExchange exchange) throws IOException {
        ...
        if (pathParams == null || pathParams.length != 4
                || !HttpUtils.containsKeys(formDataMap, "accAddress", "ownAccountId", "amount")) {
        ...
        try {
            Map<Integer, String> apiResponse = accService.initiateExternalAccountWithdrawal(
                    formDataMap.get("ownAccountId"),
                    formDataMap.get("accAddress"), userId, Double.parseDouble(formDataMap.get("amount")));
